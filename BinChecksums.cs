...
public sealed class BinCrcRegion : IEquatable<BinCrcRegion>
{
    ...
    public BinCrcRegion(int checksumOffset, int start, int end) { ChecksumOffset=checksumOffset; Start=start; End=end; }
    public bool Equals(BinCrcRegion other) { return other != null && ChecksumOffset==other.ChecksumOffset && Start==other.Start && End==other.End; }
    public override bool Equals(object other) { return Equals(other as BinCrcRegion); }
    public override int GetHashCode() { return ChecksumOffset ^ Start ^ End; }
}
public sealed class BinCrcResult
{
    ...
    { Data=data; Messages=messages; CorrectedCount=correctedCount; SkippedCount=skippedCount; }
}
...
public static class BinChecksums
{
    ...
    {
        uint result=0;
        for (int i=0;i<4;i++) result |= (uint)bytes[offset+i] << (generation==BinGeneration.Gen1 ? (3-i)*8 : i*8);
        return result;
    }
    private static void WriteStored(byte[] bytes, int offset, uint crc, BinGeneration generation)
    {
        for (int i=0;i<4;i++) bytes[offset+i]=(byte)(crc >> (generation==BinGeneration.Gen1 ? (3-i)*8 : i*8));
    }
    ...
    {
        uint crc = 0xFFFFFFFF;
        if (bytes == null || offset < 0 || length < 0 || (long)offset + length > bytes.Length) throw new ArgumentOutOfRangeException();
        for (int i=offset;i<offset+length;i++) crc = (crc >> 8) ^ Table[(crc ^ bytes[i]) & 0xFF];
        return ~crc;
    }
    private static bool Fits(BinCrcRegion region, int length) =>
        region.Start >= 0 && region.End >= region.Start && region.End < length &&
        region.ChecksumOffset >= 0 && (long)region.ChecksumOffset + 4 <= length;
    private static bool CoversChecksum(BinCrcRegion cover, BinCrcRegion stored) =>
        (long)stored.ChecksumOffset <= cover.End && (long)stored.ChecksumOffset + 3 >= cover.Start;
    ...
    public static BinCrcResult Correct(byte[] input, BinGeneration generation, IEnumerable<BinCrcRegion> verifiedDescriptors = null)
    {
        ...
        {
            if (Fits(region, input.Length)) regions.Add(region);
            else
            {
                skipped++;
        ...
        {
            if (!Fits(descriptor, input.Length)) throw new InvalidDataException("Verified checksum descriptor is outside the BIN; save cancelled.");
            if (regions.Contains(descriptor)) continue;
            regions.Add(descriptor);
        }
        if (regions.Count == 0) throw new InvalidDataException("BIN is too short for any configured checksum region; save cancelled.");
        ...
        while (pending.Count > 0)
        {
            ...
            if (next is null) throw new InvalidDataException("Cyclic checksum dependencies; save cancelled.");
            ordered.Add(next);
            pending.Remove(next);
        }
